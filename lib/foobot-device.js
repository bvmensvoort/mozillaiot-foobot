...
class FoobotDevice extends Device {
	...
	constructor(adapter, uuid, deviceinfo) {
		...
		let apiconfig = Object.assign(deviceinfo, adapter.config);
	 ...
	 */
	poll() {
		const promise = new Promise((resolve, reject) => { this.FoobotAPI.getLatestValues(() => { resolve() }); })
		...
		promise.then(() => {
			...
			let promises = propertynames.map((propertyname) => {
